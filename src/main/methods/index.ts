...
export const initMethods = (mainWindow, momentsWindow) => {
  ipcMain.on('window-moved', (_event, screenWidth, screenHeight, newBounds, xBuffer, yBuffer) => {
    if (
      newBounds.x + newBounds.width < screenWidth - 10 &&
      newBounds.x > 10 &&
      newBounds.y > 10 &&
      newBounds.y + newBounds.height < screenHeight - 10
    ) {
    ...
    if (
      newBounds.x + newBounds.width >= screenWidth - 10 &&
      xBuffer < newBounds.x &&
      newBounds.x + newBounds.width < screenWidth
    ) {
    ...
    if (
      newBounds.y + newBounds.height >= screenHeight - 10 &&
      yBuffer < newBounds.y &&
      newBounds.y + newBounds.height < screenHeight
    ) {
